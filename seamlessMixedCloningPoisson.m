function [result] = seamlessMixedCloningPoisson(source, destination, mask)
...
[destinationR destinationG destinationB] = getRGB(destination);
[maskW maskH channels] = size(mask);
...
[sourceR sourceG sourceB] = getRGB(source);
  ...
  for i = 2:srcW-1
    for j = 2:srcH-1
        ...
        sumNeighborsR = buildGuidanceVect(sourceR, destinationR, i, j);
        sumNeighborsG = buildGuidanceVect(sourceG, destinationG, i, j);
        sumNeighborsB = buildGuidanceVect(sourceB, destinationB, i, j);
 ...
 function sumNeighbors = buildGuidanceVect(sourceChannel, destChannel, x, y)
     
        % construct the guidance field	            
           
           %1st term
           if( abs(sourceChannel(x, y) - sourceChannel(x-1,y)) > abs(destChannel(x,y) - destChannel(x-1,y)))
            term1 = sourceChannel(x, y) - sourceChannel(x-1,y); 
           else
            term1 = destChannel(x,y) - destChannel(x-1,y);
           end
           ...
           if(abs(sourceChannel(x,y) - sourceChannel(x+1,y)) > abs(destChannel(x,y) - destChannel(x+1,y)))
            ...
            term2 = destChannel(x,y) - destChannel(x+1,y);
           end
           
           %3d term 
           if(abs(sourceChannel(x,y) - sourceChannel(x,y-1)) > abs(destChannel(x,y) - destChannel(x,y-1)))
            term3 = sourceChannel(x,y) - sourceChannel(x,y-1);
           else
            term3 = destChannel(x,y) - destChannel(x,y-1);
           end
           ...
           if(abs(sourceChannel(x,y) - sourceChannel(x,y+1)) > abs(destChannel(x,y) - destChannel(x,y+1)))
            ...
            term4 = destChannel(x,y) - destChannel(x,y+1);
           end
